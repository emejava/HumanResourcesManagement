...
public class PaymentService {
    ...
    public List<Payment> findByYear(Year year, User user) throws Exception {
        try (CrudRepository<Payment, Long> paymentDa = new CrudRepository<>()) {
            ...
            List<Payment> payments = paymentDa.executeQuery("payment.findByFirstAndLastName", params);
    ...
    public Payment findByYearAndMonth(Year year, Date from,Date till, User user) throws Exception {
        try (CrudRepository<Payment, Long> paymentDa = new CrudRepository<>()) {
            Map<String, Object> params = new HashMap<>();
            params.put("year", year);
            params.put("from", from);
            params.put("till", till);
            List<Payment> payments = paymentDa.executeQuery("payment.findByPhoneNo", params);
